...
Процедура ПриКомпоновкеРезультата(ДокументРезультат, ДанныеРасшифровки, СтандартнаяОбработка)
	...
	//КомпоновщикНастроекОсновнойТаблицы = Новый КомпоновщикНастроекКомпоновкиДанных;
	//КомпоновщикНастроекОсновнойТаблицы.Инициализировать(Новый ИсточникДоступныхНастроекКомпоновкиДанных(СхемаКонфигурацииОсновнаяТаблица));
	//
	////Если Настройки <> Неопределено Тогда
	////	КомпоновщикНастроекОсновнойТаблицы.ЗагрузитьНастройки(Настройки);
	////	КомпоновщикНастроекОсновнойТаблицы.Восстановить(СпособВосстановленияНастроекКомпоновкиДанных.ПроверятьДоступность);
	////Иначе
	////	КомпоновщикНастроекОсновнойТаблицы.ЗагрузитьНастройки(СхемаКонфигурацииОсновнаяТаблица.НастройкиПоУмолчанию);
	////КонецЕсли;
	////Для основной таблицы нужно чтобы отборы были из первой компановки, 
	////а поля из второй.
	////СхемаКонфигурацииОсновнаяТаблица.НастройкиПоУмолчанию 
	//
	////КомпоновщикНастроекОсновнойТаблицы.Настройки.Структура.Очистить();
	////
	////ГруппировкаДетальныеЗаписи = КомпоновщикНастроек.Настройки.Структура.Добавить(Тип("ГруппировкаКомпоновкиДанных"));
	////ГруппировкаДетальныеЗаписи.Использование = Истина;
	////
	////КомпоновщикНастроекОсновнойТаблицы.Настройки.Выбор.Элементы.Очистить(); 
	////Для каждого ПолеВыбора Из СхемаКонфигурацииОсновнаяТаблица.НастройкиПоУмолчанию.Выбор.Элементы  Цикл
	////	ДобавитьВыбранноеПолеСКД(ГруппировкаДетальныеЗаписи, ПолеВыбора);
	////КонецЦикла;
	//
	//
	//КомпоновщикМакетаОсновнойТаблицы = Новый КомпоновщикМакетаКомпоновкиДанных;
	//МакетКомпоновкиОсновнойТаблицы = КомпоновщикМакетаОсновнойТаблицы.Выполнить(СхемаКонфигурацииОсновнаяТаблица, КомпоновщикНастроекОсновнойТаблицы.ПолучитьНастройки(), , , Тип("ГенераторМакетаКомпоновкиДанныхДляКоллекцииЗначений"), Ложь);
	//
	//ПроцессорКомпоновкиОсновнойТаблицы = Новый ПроцессорКомпоновкиДанных;
	//ПроцессорКомпоновкиОсновнойТаблицы.Инициализировать(МакетКомпоновкиОсновнойТаблицы,,,Истина);
	//ПроцессорВывода = Новый ПроцессорВыводаРезультатаКомпоновкиДанныхВКоллекциюЗначений;
	//
	//ДанныеОтчета = Новый ТаблицаЗначений();
	//ПроцессорВывода.УстановитьОбъект(ДанныеОтчета);
	//ДанныеОтчета = ПроцессорВывода.Вывести(ПроцессорКомпоновкиОсновнойТаблицы);
	...
	////  {{КАМИ #349342 Борзенкова		реестры выплат (мотивация НКД) 27.11.2023             
	////Переопределяем параметры, почему-то не работает в скд
	//Настройки.ПараметрыДанных.НайтиЗначениеПараметра(Новый ПараметрКомпоновкиДанных("СрокРемонта")).Значение = КАМИ_РаботаСКонстантамиПовтИсп.ПолучитьЗначениеКонстантыКАМИ("СрокРемонта");
	//Настройки.ПараметрыДанных.НайтиЗначениеПараметра(Новый ПараметрКомпоновкиДанных("ШтрафЗаДень")).Значение = КАМИ_РаботаСКонстантамиПовтИсп.ПолучитьЗначениеКонстантыКАМИ("ШтрафПросрочка");
	////  КАМИ #349342 Борзенкова		реестры выплат (мотивация НКД) 27.11.2023}}
	//
	//Запрос = Новый Запрос;
	//Запрос.Текст = 
	...
	////|			И КАМИ_РемонтОборудованияОбороты.Назначение = ТоварыНаСкладахОстатки.Назначение
	//|ГДЕ
	//|	ИСТИНА
	//|	И ВЫБОР
	...
	//Запрос.УстановитьПараметр("ДатаОтчета", ?(ЗначениеЗаполнено(ДатаОтчета), КонецДня(ДатаОтчета), ДатаОтчета));
	//
	...
	//Перенесено в общие процедуры с АРМ
