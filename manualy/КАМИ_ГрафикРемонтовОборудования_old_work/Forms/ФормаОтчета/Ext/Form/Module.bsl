...
//  {{КАМИ #342713 Борзенкова		НКД 23.10.2023
#Область ГрафикРемонтаОборудованияЛегенда
&НаКлиенте
Процедура Легенда(Команда)        
	
	ПараметрыЛегенды = ПолучитьТекущееУсловноеОформление();                    
	
	ОткрытьФорму("Отчет.КАМИ_ГрафикРемонтовОборудования.Форма.Легенда",
	Новый Структура("СтруктураЦвета",ПараметрыЛегенды),ЭтаФорма,Новый УникальныйИдентификатор);
	
КонецПроцедуры 

&НаСервере
Функция ПолучитьТекущееУсловноеОформление()     
	Настройки = Отчет.КомпоновщикНастроек.ПолучитьНастройки();	
	ЭлементыОфрмления = Настройки.УсловноеОформление.Элементы;     
	МассивДанныхОформления = Новый Массив;    
	Для каждого ЭлементОформ Из ЭлементыОфрмления Цикл  
		Если ЗначениеЗаполнено(ЭлементОформ.ПредставлениеПользовательскойНастройки) Тогда    
			МассивДанныхСтрокиЦвета = Новый Массив;
			МассивДанныхСтрокиЦвета.Добавить(ЭлементОформ.ПредставлениеПользовательскойНастройки);
			МассивДанныхСтрокиЦвета.Добавить(ЭлементОформ.Оформление.Элементы.Найти(Новый ПараметрКомпоновкиДанных("ЦветФона")).Значение);
			МассивДанныхОформления.Добавить(МассивДанныхСтрокиЦвета); 
		КонецЕсли;
	КонецЦикла;   
	Возврат МассивДанныхОформления;
КонецФункции
